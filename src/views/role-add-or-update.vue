<template>
	<el-dialog :title="!dataForm.id ? '新增' : '修改'" v-if="isAuth(['ROOT', 'ROLE:INSERT', 'ROLE:UPDATE'])"
		:close-on-click-modal="false" v-model="visible" custom-class="dialog" width="692px">
		<el-form :model="dataForm" ref="dataForm" :rules="dataRule" label-width="60px">
			...
			<el-form-item label="权限" prop="permissions">
				<el-transfer v-model="dataForm.permissions" :data="permisionList" size="medium" :titles="['权限列表', '具备权限']"
		...
		dataFormSubmit: function () {
			...
			this.$refs['dataForm'].validate(valid => {
				if (valid) {
					...
					// 判断用户选择的权限和原来的权限是否一致，吧数组转换成字符串，简化比较两个数组是否一致
