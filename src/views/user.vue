	...
	<div v-if="isAuth(['ROOT', 'USER:SELECT'])">
		<el-form :inline="true" :model="dataForm" :rules="dataRule" ref="dataForm">
			...
			<el-form-item>
				...
				<el-button size="medium" type="primary" :disabled="!isAuth(['ROOT', 'USER:INSERT'])" @click="addHandle()">
					新增
				</el-button>
		...
		<el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle"
			...
			<el-table-column header-align="center" align="center" width="150" label="操作">
				<template #default="scope">
					...
					<el-button type="text" size="medium" v-if="isAuth(['ROOT', 'USER:UPDATE'])"
						:disabled="scope.row.status == '离职' || scope.row.root" @click="dimissHandle(scope.row.id)">
						离职
					</el-button>
...
export default {
	...
	methods: {
		...
		selectionChangeHandle: function (val) { // 传入的val是一个数组
			this.dataListSelections = val;
		},
		loadDataList: function () {
			let that = this;
			// 请i求后端数据，让表格循环滚动等待加载的图标
		...
		searchHandle: function () {
			this.$refs['dataForm'].validate(valid => {
				if (valid) {
					...
					// 不允许上传空字符串给后端，但是可以允许 null
					if (this.dataForm.name = '') {
						this.dataForm.name = null;
					}
					// 如果当前页不是第一页，就跳转到第一页显示查询的结果
					if (this.pageIndex != 1) {
						this.pageIndex = 1
					}
					this.loadDataList()
